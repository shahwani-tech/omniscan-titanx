...
export interface IdCardFilterNumericInputProps {
  id: string;
  value: number;
  min: number;
  max: number;
...
export const IdCardFilterNumericInput: React.FC<IdCardFilterNumericInputProps> = ({
  id,
  value,
  min,
  max,
  ...
  // Synchronize with external value changes (slider drags, presets, resets, side swaps)
  useEffect(() => {
    lastCommittedValueRef.current = value;
    if (!isFocused) {
      setText(formatDisplayValue(value, precision));
    } else {
      // If focused, only update if the external value differs from the parsed current input
  ...
  // Handle typing inside the numeric input
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const newText = e.target.value;
    setText(newText);

    // Allow intermediate states while actively typing without committing NaN or resetting
    ...
    if (!isNaN(parsed) && isFinite(parsed)) {
      // If within allowed min/max range, immediately update slider and preview
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    // Prevent keydown from bubbling to modal shortcuts (like Escape to close modal) or canvas
    e.stopPropagation();
  ...
  return (
    ...
    >
      <input
        ...
        onWheel={handleWheel}
        className="w-14 h-6 px-1.5 py-0.5 text-right font-mono text-[11px] tabular-nums bg-neutral-950 border border-neutral-700/80 hover:border-neutral-600 focus:border-sky-500 rounded text-neutral-100 placeholder-neutral-500 focus:outline-none focus:ring-1 focus:ring-sky-500/40 transition-colors"
      />
      {unit && (
