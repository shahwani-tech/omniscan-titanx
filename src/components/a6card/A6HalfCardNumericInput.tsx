...
export interface A6HalfCardNumericInputProps {
  id: string;
  value: number;
  min: number;
  max: number;
...
export const A6HalfCardNumericInput: React.FC<A6HalfCardNumericInputProps> = ({
  id,
  value,
  min,
  max,
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Allow intermediate states while typing without committing NaN
  ...
  // Keyboard controls: Enter to commit, Escape to revert, Up/Down to step
  ...
  return (
    ...
    >
      <input
        ...
        onWheel={handleWheel}
        className="w-14 h-6 px-1.5 py-0.5 text-right font-mono text-[11px] tabular-nums bg-neutral-950 border border-neutral-700/80 hover:border-neutral-600 focus:border-indigo-500 rounded text-neutral-100 placeholder-neutral-500 focus:outline-none focus:ring-1 focus:ring-indigo-500/40 transition-colors"
      />
      {unit && (
